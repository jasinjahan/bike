...
const WishList = () => {
  ...
  const totalPrice = wishItems.reduce((acc, item) => {
    acc += item.bikePrice * item.quantity;
    return acc;
  }, 0);
  ...
  const confirmBikeDelete = () => {
    dispatch(removeWishItem(deleteBikeId));
    toast.success("Item removed from wishlist!");
            ...
            <tbody>
              {wishItems.map((item, i) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <Image
                      src={item.bikePhoto || "/placeholder.png"}
                      className="image-photosize1"
                      alt={item.bikeName}
                    />
                  </td>

                  <td>{item.bikeName ?? 0}</td>

                  <td>
                    {item.bikePrice ?? 0} x {item?.quantity ?? 1} ={" "}
                    {item?.bikePrice * item?.quantity}
                  </td>

                  <td>{item.bikeDescription}</td>
                  ...
                  <td className="icon-align text-center">
                    <MdDelete
                      size={22}
                      className="icon-align text-center text-danger"
                      style={{ cursor: "pointer" }}
                      onClick={() => handleBikeDelete(item.id)}
                    />
                  </td>
                </tr>
              ))}
